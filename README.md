...
测试副作用隔离样板
对比实验记录器
generated workflow 审核记录
能力质量评分
```

### 当前阶段判断

截至 Stage 18，MetaCode 的核心功能闭环已经基本实现：

```text
能力单元 -> Workflow -> 执行日志 -> 失败诊断 -> 修复/规划 -> 修复事件链 -> Dashboard/API -> 测试基线 -> 对比实验 -> 审核治理 -> 能力质量评分
```

下一阶段的重点不再是继续堆基础能力，而是用真实任务验证项目价值、降低测试和演示数据噪声、把能力质量反馈给 workflow 选择和修复策略，并建立更受控的 generated workflow 提升流程。
...
tests/                Stage 2-18 自动测试
experiments/          MetaCode vs 传统 AI 编程对比实验记录
reviews/              generated workflow 审核记录
docs/                 阶段验证报告和功能设计文档
```
...
comparison_experiment_count: 2
generated_workflow_review_count: 1
```
...
The project has reached Stage 18:
...
Stage 14: one-click repair API and repair detail panel
Stage 15: test side-effect isolation and verification baseline
Stage 16: comparison experiment recorder MVP
Stage 17: generated workflow review and controlled promotion flow
Stage 18: capability quality scoring and reuse-benefit analysis
```
...
repair detail panel
test side-effect isolation baseline
comparison experiment records
generated workflow review records
capability quality scoring
```

### Current Status

As of Stage 18, the core MetaCode loop is largely implemented:

```text
capability units -> workflows -> execution logs -> failure diagnosis -> repair/planning -> repair event chains -> Dashboard/API -> testing baseline -> comparison experiments -> review governance -> capability quality scoring
```

The next work is less about adding core primitives and more about validating value with real tasks, reducing metric noise from tests and demos, feeding capability-quality scores back into workflow selection, and keeping generated workflow promotion controlled.
...
tests/                Stage 2-18 automated tests
experiments/          MetaCode vs traditional AI programming comparison records
reviews/              Generated workflow review records
